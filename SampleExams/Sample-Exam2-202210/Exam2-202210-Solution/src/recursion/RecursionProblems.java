...
public class RecursionProblems {
	...
	public static int numPairs(String input) {
		...
		if(first == last)
			toAdd = 1;
		return toAdd + numPairs(input.substring(1, input.length()-1));		
	}
	...
	public static boolean equals(String input1, String input2) {
		...
		if(input1.charAt(0) != input2.charAt(0))
			...
			return equals(input1.substring(1), input2.substring(1));
	}
	...
	public static boolean almostEquals(String input1, String input2, int threshold) {
		return almostEqualsHelper(input1, input2, 0, threshold);
	}
	
	private static boolean almostEqualsHelper(String input1, String input2, int numExceptions, int threshold) {
		if(input1.isEmpty() && input2.isEmpty() && numExceptions <= threshold)
			return true;
		if(input1.isEmpty()) {
			if((numExceptions+input2.length()) <= threshold)
				return true;
			else
				return false;
		}	
		if(input2.isEmpty()) {
			if((numExceptions+input1.length()) <= threshold)
				return true;
			else 
				return false;
		}
		if(numExceptions > threshold)
			return false;
		
		if(input1.charAt(0) != input2.charAt(0)) 
			numExceptions++;
			
		return almostEqualsHelper(input1.substring(1), input2.substring(1), numExceptions, threshold);
	}
	...
	private static boolean horizontalWordFinderHelper(char[][] puzzle, String word, int wordInd, int curRow, int curCol) {
		if(wordInd == word.length())
			return true;
		if(curRow >= puzzle.length)
			return false;
		
		if(puzzle[curRow][curCol] != word.charAt(wordInd))
			return false;
		else {
			if(curCol == puzzle[0].length-1) {
				curCol = 0;
				curRow++;
			} else
				curCol++;
			return horizontalWordFinderHelper(puzzle, word, wordInd+1, curRow, curCol);
		}
	}

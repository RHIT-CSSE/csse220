...
public class MapAnd2DArrayProblems {
	...
	public static int [][] putTogether(int[][] a1, int [][] a2) {
		...
		int rowMax = rowsA1 + rowsA2;
		int cols = a1[0].length;
		int [][] newA = new int[rowMax][cols];
		...
		for (int newArow = rowsA1, a2Row = 0; newArow < rowMax; newArow++, a2Row++) {
			for (int c = 0; c < cols; c++) {
				newA[newArow][c] = a2[a2Row][c];
			}
		}		
		return newA;
	}
	...
	public static ArrayList<Integer> findKeysThatMatchesValues(HashMap<Integer,String> map, HashSet<String> valuesToFind) {
		...
		for (Integer key : map.keySet()) {
			if (valuesToFind.contains(map.get(key))){
				keys.add(key);
			}
		}
		return keys;		
